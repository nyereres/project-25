...
extern int rightDistance, middleDistance, leftDistance;
...
void left_hand_rule() {
    ...
    set_degree(170);
    delay_ms(500);
    leftDistance = distance(Trig, Echo); // left distance

    if (rightDistance > 10) {
        // left is open, go right
        Motor(StearLeft, 70);
        delay_ms(500);
    } else if (middleDistance > 10) {
        // middle is open, go straight
        ...
        // middle and left are closed, go right
        Motor(StearRightt, 70);
        delay_ms(500);
    }
}